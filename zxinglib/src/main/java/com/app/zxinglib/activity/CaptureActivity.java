...
import com.app.zxinglib.camera.CameraManager;
import com.app.zxinglib.decode.DecodeUtils;
import com.app.zxinglib.utils.BeepManager;
import com.app.zxinglib.utils.CaptureActivityHandler;
import com.app.zxinglib.utils.InactivityTimer;
import com.app.zxinglib.utils.UriUtils;
...
public final class CaptureActivity extends AppCompatActivity implements SurfaceHolder.Callback {
    ...
    public static final int PERMISSIONS_REQUEST_CODE_READ_EXTERNAL_STORAGE = 102;
    ...
    public static void startActivityForResult(Activity mActivity, int requestCode, boolean needCallBack){
        Intent openCameraIntent = new Intent(mActivity, CaptureActivity.class);
        openCameraIntent.putExtra(SCAN_RESULT_CALLBACK, needCallBack);
    ...
    private boolean requestPermission(String permission, int requestCode) {
        if (ActivityCompat.checkSelfPermission(CaptureActivity.this, permission)
                != PackageManager.PERMISSION_GRANTED){
            String[] permissions = new String[1];
            permissions[0] = permission;
            ActivityCompat.requestPermissions(CaptureActivity.this, permissions, requestCode);
            return true;
        }
        return false;
    }
    ...
    private void startPreview(boolean checkPermission) {
        if (checkPermission && requestPermission(Manifest.permission.CAMERA,
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions, @NonNull int[] grantResults) {
        ...
        if (requestCode == PERMISSIONS_REQUEST_CODE_CAMERA) {
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED) {
                ...
                Toast.makeText(CaptureActivity.this, "本功能需要打开相机权限！", Toast.LENGTH_SHORT).show();
            }
        }else if(requestCode == PERMISSIONS_REQUEST_CODE_READ_EXTERNAL_STORAGE){
            if (grantResults.length > 0 && grantResults[0] == PackageManager.PERMISSION_GRANTED){
                getPicFromGallery();
            }else {
                Toast.makeText(CaptureActivity.this, "本功能需要打开读取本地图库权限！", Toast.LENGTH_SHORT).show();
            }
        }
    }
